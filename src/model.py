...
class HybridGraphSAGE(nn.Module):
    ...
    def __init__(
        ...
        # BatchNorm before fusion (normalizes scale of both paths)
    ...
    def forward(self, x, edge_index, batch, global_feat):
        ...
        # Normalize both paths to same scale before fusion
...
class TextAwareGraphSAGE(nn.Module):
    """Two-layer GraphSAGE with CNN+GNN hybrid fusion and text density features.

    Extends HybridGraphSAGE by accepting node features that include a text density
    channel extracted from doctr DBNet. The only architectural difference is the
    default node_dim (2051 = 2048 CNN + 2 PE + 1 text_density vs 2050 for hybrid).
    ...
    Architecture:
        GNN path: SAGEConv(node_dim, hidden) -> ReLU -> Dropout
                  ...
                  -> global_mean_pool -> gnn_embed [batch_size, embed_channels]
        CNN path: global_feat [batch_size, global_channels]
        Fusion: BatchNorm(gnn_embed) + BatchNorm(global_feat)
                -> concat -> Linear(fusion_dim, num_classes)

    Args:
        node_dim: Input node feature dimension (2048 CNN + 2 PE + 1 text_density = 2051)
        hidden_channels: First SAGEConv output dimension
        embed_channels: Second SAGEConv output dimension (GNN embedding size)
        global_channels: CNN global feature dimension (ResNet-50 avgpool = 2048)
        num_classes: Number of classification categories
